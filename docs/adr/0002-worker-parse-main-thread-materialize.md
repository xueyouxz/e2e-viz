...
2. **Main thread** (`SceneRepository` materialization):
...
- The `RawDecodedFrame` type (with `_raw` discriminant) is the Worker IPC contract — changes require updating `frameDecoderMessages.ts`, `FrameDecoder.ts`, and the main-thread materialization path
