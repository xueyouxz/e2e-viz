...
import type { RawMetadataParseResult } from '../MessageParser'
import type { ParsedFrame, SceneMetadata } from '../../types'
...
vi.mock('../MessageParser', () => {
  return {
    parseMetadata: parserMocks.parseMetadata,
    MessageParser: class {
      parseFrame = parserMocks.parseFrame
...
function frame(index: number): ParsedFrame {
  if (index === 9) {
    return {
...
async function initLoader(loader: SceneLoader): Promise<void> {
  await loader.init()
}
...
beforeEach(() => {
  ...
  parserMocks.parseFrame.mockImplementation((buffer: ArrayBuffer) => {
...
describe('SceneLoader', () => {
  ...
  it('deduplicates and promotes a queued user request ahead of background prefetch', async () => {
    ...
    const critical = loader.loadFrame(5)
    const duplicate = loader.loadFrame(5)
  ...
  it('revokes every materialized image URL on destroy', async () => {
    parserMocks.parseMetadata.mockReturnValueOnce({
      metadata,
      initialStreamState: {
        '/map/basemap': {
          _raw: 'image',
    ...
    loader.subscribeCacheChanges(onCacheChange)
    await initLoader(loader)
    await loader.loadFrame(9)
  ...
  it('does not materialize a parsed frame that completes after destroy', async () => {
    installImmediateFetch()
    const loader = new SceneLoader('/scene/')
    await initLoader(loader)
    const pendingParse = deferred<ParsedFrame>()
    parserMocks.parseFrame.mockImplementationOnce(() => pendingParse.promise)

    const pendingLoad = loader.loadFrame(9)
    await vi.waitFor(() => expect(parserMocks.parseFrame).toHaveBeenCalledTimes(2))
    loader.destroy()
    pendingParse.resolve(frame(9))

    await expect(pendingLoad).rejects.toThrow('SceneLoader destroyed')
