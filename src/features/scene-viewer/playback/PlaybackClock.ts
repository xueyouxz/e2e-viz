...
export function advancePlaybackClock(
  ...
  if (timestamps.length === 0) return { timeSeconds: 0, frameIndex: 0, isAtEnd: true }
  ...
  return {
    timeSeconds,
    frameIndex: findNearestFrameIndex(timestamps, timeSeconds),
    isAtEnd: timeSeconds >= endTime
  }
}
