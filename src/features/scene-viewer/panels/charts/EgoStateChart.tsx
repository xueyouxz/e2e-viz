...
export function EgoStateChart({ egoSpeed, egoAcceleration, frameCount }: EgoStateChartProps) {
  ...
  const {
    ...
    const accelAbs = Math.max(
      Math.abs(arrayMax(egoAcceleration)),
      Math.abs(arrayMin(egoAcceleration)),
      0.1
    )
  ...
  const speedKmhFormat = useMemo(() => (v: number) => `${(v * 3.6).toFixed(0)}`, [])

  const accelMsFormat = useMemo(() => (v: number) => `${v >= 0 ? '+' : ''}${v.toFixed(1)}`, [])
  ...
  return (
    ...
    >
      {/* ── Speed ─────────────────────────────────────────────────────────────── */}
      <text {...tick} x={ML - 4} y={SP_TOP}>
        {speedMaxLabel}
      </text>
      <text {...tick} x={ML - 4} y={SP_BOT}>
        0
      </text>
      <text {...unit} x={ML + PLOT_W} y={SP_LABEL_Y} fill={SPEED_COLOR}>
        km/h
      </text>
      ...
      {/* ── Acceleration ──────────────────────────────────────────────────────── */}
      <text {...tick} x={ML - 4} y={AC_MID}>
        0
      </text>
      <text {...unit} x={ML + PLOT_W} y={AC_LABEL_Y} fill={palette.tickFill}>
        m/s²
      </text>
