...
} from '../utils/camera/projection'
import { CAMERA_CHANNELS } from '../utils/camera/types'
import type { ChannelProjectedBoxes, ProjectedBox3DWireframe } from '../utils/camera/types'
...
export function useCameraProjectedBoxes(
  cuboidStreamName = '/gt/objects/bounds'
): ChannelProjectedBoxes {
  const cuboidPayload = useSceneStore(
    s => s.streamState[cuboidStreamName] as CuboidPayload | undefined
  )
