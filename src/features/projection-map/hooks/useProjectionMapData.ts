...
function buildSplitLookup() {
  ...
  for (const split of Object.keys(VECTOR_MAP_MANIFEST.splits) as SplitName[]) {
    for (const sceneName of VECTOR_MAP_MANIFEST.splits[split]) {
      lookup.set(sceneName, split)
    }
  }
  return lookup
}
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const splitLookup = buildSplitLookup()
        const summaryLookup = new Map(
          objectSummary.scenes.map(scene => [scene.scene_name, scene])
        )
        const nextPoints = projection.scenes
          .map(scene => {
            const split = splitLookup.get(scene.scene_name)
  ...
  const splitCounts = useMemo(
    () =>
      points.reduce<Record<SplitName, number>>(
        (counts, point) => { counts[point.split] += 1; return counts },
        { train: 0, val: 0 }
      ),
    [points]
  )
